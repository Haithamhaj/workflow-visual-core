...
import type { WorkflowGraph, WorkflowGraphNode, NodeType } from "../../core/types.js";
import { escapeMermaidLabel, sanitizeNodeId } from "./escapeMermaid.js";
import type { MermaidRenderOptions } from "./mermaidTypes.js";

// ── Node shape rendering ───────────────────────────────────

function renderNodeShape(id: string, label: string, nodeType: NodeType): string {
...
function statusClass(node: WorkflowGraphNode): string | null {
...
export function toMermaid(
  ...
  if (!options.skipValidation) {
    ...
    if (!result.ok) {
      throw new ValidationFailedError(
        `Cannot render Mermaid: graph validation failed.\n` +
  ...
  lines.push("");
  lines.push("  %% Nodes");
  const nodeIdMap = new Map<string, string>(); // original id → sanitized id
